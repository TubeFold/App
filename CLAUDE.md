...
- `providers/codex.sh` delegates to `scripts/run-codex-provider.py`, which runs `codex exec` from an **isolated temp dir**, reads the prompt from **stdin**, and captures the model's last message via `--output-last-message`. Codex runs with `--sandbox read-only --ephemeral --ignore-rules --skip-git-repo-check` so it can't touch the repo or network state.
- `providers/claude.sh` delegates to `scripts/run-claude-provider.py`, which runs `claude --print --model <m> [--effort <e>] --output-format text` from an **isolated temp dir** (so the CLI never discovers a CLAUDE.md), reads the prompt from **stdin**, and captures the model's text response from **stdout**. Uses the user's Claude subscription (OAuth/keychain), not an API key — `--bare` is deliberately *not* used.
