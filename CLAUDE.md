...
Tool that turns a YouTube URL into a Markdown summary. It does **not** use the OpenAI API — the default provider shells out to the locally-installed `codex` CLI (the user's own Codex subscription) and keeps only the model's final message. The README is in Russian; this file and the code are in English.
...
It must write **only the final Markdown** to `<output_file>` and exit `0`. Config values are passed to the provider via environment variables (e.g. `CODEX_MODEL`, `CODEX_REASONING_EFFORT`, `CODEX_TIMEOUT_SECONDS`).
...
- `providers/fake.sh` emits canned Markdown (overridable via `$FAKE_PROVIDER_OUTPUT`). Use it for tests and any pipeline work that shouldn't burn a real Codex run.

To add a provider, drop an executable `providers/<name>.sh` honoring the contract — no other registration needed.
...
- MVP scope: no playlists, no Whisper fallback, no chunked summarization (long transcripts are sent whole), Codex-only provider.
- The `codex` CLI is an external user dependency even in the packaged app; only the Python backend is bundled.
